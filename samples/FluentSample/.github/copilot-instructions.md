...
Be sure the user has Power Apps CLI installed and is signed in. Use `pa auth status` to
verify the active user, and read `environmentId` from `power.config.json` to verify the
...
```bash
pa app add data-source --connector "shared_office365users" --connection-id <connectionId>
```
...
```bash
pa app add data-source --connector "shared_sql" --connection-id <connectionId> \
...
```bash
pa app add data-source --connector "shared_customapi" --connection-id <connectionId>
```
...
Create or select a Power Platform environment with Code Apps enabled in the Power Platform
admin center.
...
```bash
pa app init --environment-id <environment-id> --display-name "My App" --logo-path "[location of the vite.svg]"
```
...
1. **Create new service file** (auto-generated by Power Apps CLI)
