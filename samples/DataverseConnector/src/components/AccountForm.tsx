 ...
 * - Edit mode includes an Attachment sub-form that uploads via the Dataverse connector
 *
 * COMPONENT ARCHITECTURE:
 * - Pure presentational component for the main form fields
 * - Attachment sub-form calls the connector wrapper (uploadFileImage/downloadFile) directly
 *   (upload/download are one-off actions that belong at the component boundary, not in the CRUD hook)
 * - Toast notification surfaces success/error for 5 seconds
 */

import { useState, useEffect, useRef } from 'react';
import { createPortal } from 'react-dom';
import type { Accounts, AccountsUploadColumnName, AccountsImageColumnName } from '../types';
import { ACCOUNT_FILE_COLUMN, ACCOUNT_IMAGE_COLUMN } from '../types';
import {
  uploadFileImage,
  downloadFile,
  downloadFileImage,
...
export function AccountForm({
  ...
  const [columnName, setColumnName] = useState<AccountsUploadColumnName>(ACCOUNT_FILE_COLUMN);
  ...
  useEffect(() => {
    ...
    setColumnName(ACCOUNT_FILE_COLUMN);
    setFileDisplayName('');
  ...
  const handleDownload = async (col: AccountsUploadColumnName, fallbackFileName: string) => {
    ...
    try {
      const { blob, fileName, size } = await downloadFile('accounts', selectedAccount.accountid, col);
      if (size > 0) {
        triggerBlobDownload(blob, fileName || fallbackFileName);
      } else {
        showToast('Download failed: no file content.', 'error');
      }
    } catch (err) {
  ...
  const handleDownloadImage = async (fallbackFileName: string) => {
    if (!selectedAccount?.accountid) return;
    const imageCol: AccountsImageColumnName = ACCOUNT_IMAGE_COLUMN;
    setDownloadingCol(imageCol);
    try {
      const base64 = await downloadFileImage('accounts', selectedAccount.accountid, imageCol, imageFullSize);
      if (base64) {
        triggerBlobDownload(base64ToBlob(base64, 'image/png'), `${fallbackFileName}.png`);
      } else {
        showToast('Image download failed: no content.', 'error');
      }
    } catch (err) {
  ...
  const handleClearFile = async (col: AccountsUploadColumnName) => {
    ...
    try {
      const cleared = await deleteFileImage('accounts', selectedAccount.accountid, col);
      if (cleared) {
        showToast('File removed successfully.', 'success');
        onUploadSuccess?.();
      } else {
        // File columns can't be cleared through the connector (no DELETE operation, and Dataverse
        // ignores null in a PATCH for File columns). Image columns clear fine.
        showToast(
          'The Dataverse connector cannot clear File columns (no delete operation exposed).',
          'error',
        );
      }
    } catch (err) {
  ...
  return (
    <section className="contact-form">
      {/* Toast notification — portaled to <body> so it stays fixed to the viewport,
          escaping the `transform` on the animated <section> containing block. */}
      {toast &&
        createPortal(
          <div className={`upload-toast upload-toast--${toast.type}`}>{toast.message}</div>,
          document.body,
        )}
      ...
      {!isCreating && selectedAccount && (
        <div className="attachment-subform">
          ...
          {/* Current file stored in the File column (ACCOUNT_FILE_COLUMN) */}
          <div className="form-group">
            <label>{ACCOUNT_FILE_COLUMN} (current value)</label>
            {selectedAccount[`${ACCOUNT_FILE_COLUMN}_name`] ? (
              <div className="file-current">
                <span className="file-current-icon">📄</span>
                <span className="file-current-name">{String(selectedAccount[`${ACCOUNT_FILE_COLUMN}_name`])}</span>
                <button
                  type="button"
                  className="file-action-btn"
                  onClick={() => handleDownload(ACCOUNT_FILE_COLUMN, String(selectedAccount[`${ACCOUNT_FILE_COLUMN}_name`]))}
                  disabled={downloadingCol === ACCOUNT_FILE_COLUMN}
                  title="Download file"
                >
                  {downloadingCol === ACCOUNT_FILE_COLUMN ? '…' : '⬇'}
                </button>
                <button
                  type="button"
                  className="file-clear-btn"
                  onClick={() => handleClearFile(ACCOUNT_FILE_COLUMN)}
          ...
          {/* Current image stored in the Image column (ACCOUNT_IMAGE_COLUMN, full-size storage enabled) */}
          <div className="form-group">
            <label>{ACCOUNT_IMAGE_COLUMN} (current value)</label>
            {selectedAccount[ACCOUNT_IMAGE_COLUMN] ? (
              <div className="file-current">
                ...
                <button
                  ...
                  disabled={downloadingCol === ACCOUNT_IMAGE_COLUMN}
                  title="Download image"
                >
                  {downloadingCol === ACCOUNT_IMAGE_COLUMN ? '…' : '⬇'}
                </button>
                <button
                  type="button"
                  className="file-clear-btn"
                  onClick={() => handleClearFile(ACCOUNT_IMAGE_COLUMN)}
                  disabled={isClearingFile}
                  title="Remove image"
                >
                  {isClearingFile ? '…' : '×'}
          ...
          {attachmentFile && (
            <>
              <div className="form-group">
                ...
                >
                  <option value={ACCOUNT_FILE_COLUMN}>{ACCOUNT_FILE_COLUMN}</option>
                  <option value={ACCOUNT_IMAGE_COLUMN}>{ACCOUNT_IMAGE_COLUMN}</option>
                </select>
