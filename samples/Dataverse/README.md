...
```bash
npm install
pa app init --environment-id <your-environment-id> --display-name "Dataverse Demo App"
```

### 3. Configure the Account file column

The Account attachment example needs a custom Dataverse **File** column. Publisher prefixes and
logical names differ between environments, so the sample does not hardcode one.

1. In the maker portal, create a File column on the Account table.
2. Copy its logical name, including your publisher prefix.
3. Set that value in [`src/config.ts`](src/config.ts):

```typescript
export const ACCOUNT_FILE_COLUMN = '<your-file-column-logical-name>';
```

Create the column and update the configuration before generating the Account data source. When
`ACCOUNT_FILE_COLUMN` is empty, custom file-column operations are disabled, but the standard
`entityimage` example remains available.
...
pa app add dataverse-api --api-name SetAutoNumberSeed
pa app add dataverse-api --api-name ConvertOwnerTeamToAccessTeam
```

These commands generate the models and services used by the app under `src/generated/`, plus
environment-specific files under `.power/` and `power.config.json`. Run every command before
building when those generated artifacts are not present.
...
The **Functions & Actions** tab demonstrates three API patterns generated by the Power Apps CLI.

```bash
# Search for available actions and functions
pa app find-dataverse-api --search "<name>"

# Generate a typed service
pa app add dataverse-api --api-name <OperationName>
```
...
| Issue                 | Solution                                                                    |
| --------------------- | --------------------------------------------------------------------------- |
| Authentication failed | Run `pa auth login`                                                        |
...
| [DEVELOPMENT.md](./DEVELOPMENT.md)   | Step-by-step guide to recreate this demo from scratch using the Power Apps CLI                                                        |
...
- [Power Apps Code Apps CLI](https://learn.microsoft.com/en-us/power-apps/developer/code-apps/)
